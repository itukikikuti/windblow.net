...
53～80行目はウィンドウを出す処理をしています。`CreateWindowW`という関数でウィンドウを作って、ウィンドウのサイズと位置を設定して`ShowWindow`で表示しています。

82行目の`D3D_DRIVER_TYPE`はドライバーの種類なんですが`D3D_DRIVER_TYPE_HARDWARE`がハードウェアでDirect3Dが動くので高速です。ほかの三つはソフトウェアでうごくので遅いです。なのでハードウェアが対応していればハードウェア、対応していなければソフトウェアでうごかしたいので、114行あたりで対応している中で一番速いものを探している感じです。
