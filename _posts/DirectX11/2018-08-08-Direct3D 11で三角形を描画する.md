...
1行目に`#define OEMRESOURCE`というのがありますがこれがないと`OCR_NORMAL`が定義されないのです。ということで必要なものです。

次にいろいろとインクルードしてて、関数と構造体があります。
...
その下の`_CrtSetDbgFlag`という関数を呼び出すと、メモリリークを検出できるようになります。

その下では`CreateWindowW`という関数でウィンドウを作って、ウィンドウのサイズと位置を設定して`ShowWindow`で表示しています。
