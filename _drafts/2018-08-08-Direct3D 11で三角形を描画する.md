...
category: DirectX11
---
DirectX 11を学ぶ上で一番最初にやることは三角形の描画です。Hello Triangleと言ったりもします。なぜ三角形なのかというと3DCGは三角形の集合体だからです。よくポリゴンといわれているものですね。いわば3DCGの最小単位です。最近のゲームはポリゴンが細かいので形が滑らかで三角形ぽさはないですが、昔のゲームはポリゴンポリゴンしてたので分かりやすいですね。
...
そうするとこのように三角形が回転します。それでは詳しく解説していきます。

1行目に`#define OEMRESOURCE`というのがありますがこれがないと62行目にある`OCR_NORMAL`が定義されないのです。ということで必要なものです。

次に47行目まではいろいろとインクルードしてて、関数と構造体があります。

その下には`int APIENTRY wWinMain(HINSTANCE, HINSTANCE, LPWSTR, int)`というエントリーポイントがあります。ここからプログラムが処理されるわけです。ちなみにエントリーポイントを`int main()`にするとコンソールが出ますので、printfとかを使ってデバッグができるようになります。

51行目の`_CrtSetDbgFlag`という関数は、メモリリークを検出できるようにするものです。

53行目からはウィンドウを出す処理をしています。`CreateWindowW`という関数でウィンドウを作って、ウィンドウのサイズと位置を設定して`ShowWindow`で表示しています。

82行目の`D3D_DRIVER_TYPE`はドライバーの種類なんですが`D3D_DRIVER_TYPE_HARDWARE`がハードウェアでDirect3Dが動くので高速です。ほかの三つはソフトウェアで動くので遅いです。なのでハードウェアが対応していればハードウェアで動かしたいので、114行あたりで対応している中で一番速いものを探しています。

`D3D_FEATURE_LEVEL`というのは[機能レベル](https://docs.microsoft.com/ja-jp/windows/desktop/direct3d11/overviews-direct3d-11-devices-downlevel-intro)というものがあって環境に合わせて一番いいやつを選んでくれます。本書ではDirect3D 11を使うので`D3D_FEATURE_LEVEL_11_0`を一番上にします。

`DXGI_SWAP_CHAIN_DESC`はスワップチェインというものを作るための設定をするためのものです。
